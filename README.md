# HTMLサイトにログイン認証をかけるデモ（Azure + Entra ID）
...
HTMLファイルを **Azureに直接アップロードすることも可能**です。
...
Aさん：HTMLのタイトルを変更 → GitHubにアップロード → 自動でAzureに反映
Bさん：「その変更OK！」とレビューで承認
...
**Q. 今の設定だと誰がアクセスできるの？**
→ `"allowedRoles": ["authenticated"]` の設定は、**Entra IDでログインできる人なら誰でもアクセスOK** という意味です。Microsoft社員（@microsoft.com）だけでなく、他の組織のEntra IDアカウントや個人のMicrosoftアカウント（@outlook.com 等）でもログインできます。

> ⚠️ **注意：** 今の設定は「特定の組織だけに制限」しているわけではありません。Microsoftアカウントを持っている人なら誰でもアクセスできる状態です。

| やりたいこと | 必要なプラン | 設定方法 |
|---|---|---|
| Entra ID アカウントを持つ全員がアクセス可能 | **Free**（今の状態） | `"allowedRoles": ["authenticated"]` |
| 招待した特定の人だけアクセス可能 | **Free** | ロールの管理で招待（下記参照） |
| 特定の組織（テナント）だけアクセス可能 | **Standard（有料）** | カスタム Entra ID 認証を設定 |
...
> ⚠️ ただし、Azure の利用には別途**従量課金**が発生する場合があります。詳しくは [Azure 料金計算ツール](https://azure.microsoft.com/ja-jp/pricing/calculator/) から事前にご確認ください。
